...
public class CustomProjectile {
	...
	public CustomProjectile(Entity projectile, LivingEntity sender, IProjectile iProj, Vector velocity, long expireTime,
			boolean targetLivingEntity, boolean targetBlock, LivingEntity[] entityBlacklist,
	...
	private void TransferBlackLists(LivingEntity[] entityBlacklist, Material[] blockBlacklist) {
		if (targetLivingEntity) {
			...
			for (int i = 0; i < entityBlacklist.length; i++)
				this.entityBlacklist.add(entityBlacklist[i]);
		}
		if (targetBlock) {
			this.blockBlacklist = new ArrayList<>();
			for (int i = 0; i < blockBlacklist.length; i++)
				this.blockBlacklist.add(blockBlacklist[i]);
		}
	}
					...
					if (!entityBlacklist.contains(hitLivingEntity)) {
						if (hitLivingEntity instanceof Player) {
							...
							if (hitPlayer.getGameMode() == GameMode.CREATIVE
									|| hitPlayer.getGameMode() == GameMode.SPECTATOR)
								continue;
						}
			...
			for (int x = startX; x <= endX; x++) {
				for (int y = startY; y <= endY; y++) {
					for (int z = startZ; z <= endZ; z++) {
						...
						if (!blockBlacklist.contains(hitBlock.getType()) && hitBlock.isSolid()
								&& hitBlock.getBoundingBox().overlaps(box)) {
