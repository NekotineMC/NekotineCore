...
public class ProjectileModule implements IPluginModule, Listener {
	 ...
	 *
	 * @param projectile
	 *            Entité qui sert de projectile
	 * @param sender
	 *            Lanceur du projectile
	 * @param iProj
	 * @param velocity
	 *            Velocité initiale du projectile
	 * @param expireTime
	 *            Durée de vie du projectile en ms
	 * @param targetLivingEntity
	 *            Si le projectile peut toucher les LivingEntity
	 * @param targetBlock
	 *            Si le projectile peut toucher les Block
	 * @param entityBlacklist
	 *            Entités à ignorer
	 * @param blockBlacklist
	 *            Matériaux des blocks à ignorer
	 * @return True si le projectile a été ajouté
	 */
	public boolean AddProjectile(Entity projectile, LivingEntity sender, IProjectile iProj, Vector velocity,
			long expireTime, boolean targetLivingEntity, boolean targetBlock, LivingEntity[] entityBlacklist,
			Material[] blockBlacklist) {
		if (BufferExist(projectile))
			return false;

		CustomProjectile customProjectile = new CustomProjectile(projectile, sender, iProj, velocity, expireTime,
				targetLivingEntity, targetBlock, entityBlacklist, blockBlacklist);
		projectilesBuffer.put(projectile, customProjectile);
		return true;
	}

	public boolean AddProjectile(Entity projectile, LivingEntity sender, IProjectile iProj, Vector velocity,
			long expireTime, boolean targetLivingEntity, boolean targetBlock) {
		LivingEntity[] entityBlacklist = {};
		Material[] blockBlacklist = {};
		return AddProjectile(projectile, sender, iProj, velocity, expireTime, targetLivingEntity, targetBlock,
				entityBlacklist, blockBlacklist);
	}
	...
	public void TriggerFromSender(@NotNull LivingEntity sender) {
		...
		for (Iterator<Entry<Entity, CustomProjectile>> iterator = projectiles.entrySet().iterator(); iterator
				.hasNext();) {
			Entry<Entity, CustomProjectile> entry = iterator.next();

			if (sender.equals(entry.getValue().GetSender()))
				entry.getValue().SetTriggered(true);
		}
	}
	...
	public void TriggerFromInterface(@NotNull IProjectile iProj) {
		...
		for (Iterator<Entry<Entity, CustomProjectile>> iterator = projectiles.entrySet().iterator(); iterator
				.hasNext();) {
			Entry<Entity, CustomProjectile> entry = iterator.next();
			if (iProj.equals(entry.getValue().GetInterface()))
				entry.getValue().SetTriggered(true);
		}
	}
	...
	public ArrayList<CustomProjectile> GetFromSender(@NotNull LivingEntity sender) {
		...
		for (CustomProjectile projectile : projectiles.values()) {
			if (sender.equals(projectile.GetSender()))
				senderProjectiles.add(projectile);
		}
	...
	public void Tick(TickElapsedEvent e) {
		...
		for (Iterator<Entry<Entity, CustomProjectile>> iterator = projectiles.entrySet().iterator(); iterator
				.hasNext();) {
			Entry<Entity, CustomProjectile> entry = iterator.next();
			if (!entry.getValue().GetProjectile().isValid() || entry.getValue().Collision())
				iterator.remove();
		}
	}
	...
	public void OnProjectileHit(ProjectileHitEvent e) {
		...
		if (e.getHitBlock() == null)
			return;
		if (MainExist(e.getEntity()))
			projectiles.get(e.getEntity()).ProjectileHitBlock(e.getHitBlock());
	}
