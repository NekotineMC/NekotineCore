...
public class GearUtil {
	 ...
	 *
	 * @param item
	 *            L'Itemstack � regarder
	 * @param material
	 *            Le Material � comparer
	 * @return True si �gaux
	 */
	...
	public static void Replace(Inventory inventory, ItemStack from, ItemStack to) {
		for (Integer slot : inventory.all(from).keySet())
		...
		if (inventory instanceof EntityEquipment) {
			...
			if (from.equals(equipmentInventory.getHelmet()))
				equipmentInventory.setHelmet(to);
			if (from.equals(equipmentInventory.getChestplate()))
				equipmentInventory.setChestplate(to);
			if (from.equals(equipmentInventory.getLeggings()))
				equipmentInventory.setLeggings(to);
			if (from.equals(equipmentInventory.getBoots()))
				equipmentInventory.setBoots(to);
			if (from.equals(equipmentInventory.getItemInOffHand()))
				equipmentInventory.setItemInOffHand(to);
		}
		if (inventory instanceof PlayerInventory) {
			PlayerInventory playerInventory = (PlayerInventory) inventory;

			if (from.equals(playerInventory.getHelmet()))
				playerInventory.setHelmet(to);
			if (from.equals(playerInventory.getChestplate()))
				playerInventory.setChestplate(to);
			if (from.equals(playerInventory.getLeggings()))
				playerInventory.setLeggings(to);
			if (from.equals(playerInventory.getBoots()))
				playerInventory.setBoots(to);
			if (from.equals(playerInventory.getItemInOffHand()))
				playerInventory.setItemInOffHand(to);
		}
	}
