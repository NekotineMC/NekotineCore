...
/**
 * Classe abstraite représentant un module utilisable par un plugin. Le module
 * reçois, gère et peut creér de nouveaux évenements Bukkit.
 * 
...
public abstract class PluginModule implements Listener {
	...
	/**
	 * Méhode à ne pas appeler, utilisez plutôt {@link #enable()} Cette méthode est
	 * destinée à être surchargée par la classe enfant.
	 */
	...
	protected void onEnable() {
		...
		try {
			for (Field field : getClass().getDeclaredFields()) {
				...
				if (annotation != null) {
					if (PluginModule.class.isAssignableFrom(field.getType())) {
						...
						try {
							field.set(this,
									_manager.getAbstractModule((Class<? extends PluginModule>) field.getType()));
						} catch (Exception e) {
						}
					}
				}
			...
			logException(Level.WARNING, "Une erreur est survenue lors de la recuperation des dependances de ce module",
					e);
		}
		//
	...
	/**
	 * Méhode à ne pas appeler, utilisez plutôt {@link #disbale()} Cette méthode est
	 * destinée à être surchargée par la classe enfant.
	 */
