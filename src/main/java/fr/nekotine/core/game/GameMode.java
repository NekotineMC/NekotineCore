...
import fr.nekotine.core.game.exception.GamePhaseException;
...
public abstract class GameMode<GD extends GameData> implements Listener {
	...
	/**
	 * Cette fonction est appelée à la création de la partie pour mettre en place
	 ...
	 */
	public final void GotoGamePhase(Game<GD> game, String phaseKey) {
		...
		if (phase != null) {
			if (currentGamePhase != null) {
				try {
					...
					throw new GamePhaseException(msg, e);
				}
			}
	...
	/**
	 * Méthode appelée naturellement au début de la partie. Elle permet d'aller vers
	 * la première phase.
	 */
	...
	public final boolean Start(Game<GD> game) {
		...
		ModuleManager.GetModule(GameModeModule.class).log(Level.INFO, "Une partie commence.");
		try {
			globalSetup(game);
			try {
	...
	/**
	 * Met fin à la partie. Les statistiques/données d'après match sont enregistrées
	 * et le traitement pour un quelconque classement est fait.<br>
	 * <br>
	 * Cette fonction est utile pour les modes de jeu qui ne peuvent se finir seul
	 * (un mode de jeu sans fin par exemple). Cette fonction fonctionne en plusieurs
	 * étapes définies par les fonctions suivantes:<br>
	...
	public final boolean Stop(Game<GD> game) {
		...
		// collect game data
		var plugin = CorePlugin.getCorePluginInstance();
		ModuleManager.GetModule(GameModeModule.class).log(Level.INFO, "Une partie s'arrête.");
		try {
			...
			Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
				try {
					...
					log.log(Level.WARNING,
							"Une erreur est survenue de façon asynchrone lors de la gestion des donnees de la partie",
							e);
				}
			});
		} catch (Exception e) {
	...
	/**
	 * Met fin prématurément à la partie. Aucun traitement n'est fait sur les
	 * statistiques/données d'après match, aucun traitement pour un quelconque
	 * classement est fait. La partie "compte pour du beure".
	 */
	public final boolean Abort(Game<GD> game) {
		if (!game.isPlaying())
			return false;
		var plugin = CorePlugin.getCorePluginInstance();
		ModuleManager.GetModule(GameModeModule.class).log(Level.INFO, "Une partie s'interrompt.");
		try {
			currentGamePhase.End(game);
			for (var team : game.getTeams()) {
				for (var player : team.getPlayerList()) {
	...
	/**
	 * Mise en place de la partie, abonnement aux services utilisés (pas besoin de
	 * s'abonner aux événements Bukkit) et début de celle-ci. Le chargement d'une
	 * sauvegarde ou de données peut aussi être fait ici (ou lancé de manière
	 * asynchrone depuis cette fonction) <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Start Start}
	 */
	protected abstract void globalSetup(Game<GD> game);

	/**
	 * Mise en place de chaque joueur avant le début de la partie. Cette methode est
	 * appelée pour chaque joueur. Le chargement d'une sauvegarde ou de données peut
	 * aussi être fait ici (ou lancé de manière asynchrone depuis cette fonction)
	 * <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Start Start}
	 */
	...
	/**
	 * Arret des mechaniques de jeu et désabonnement aux services utilisés. <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Stop Stop} et {@link Game#Abort
	 * Abort}
	 */
	protected abstract void globalEnd(Game<GD> game);

	/**
	 * Arret des mechaniques de jeu pour chaque joueur. Cette methode est appelée
	 * pour chaque joueur. <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Stop Stop} et {@link Game#Abort
	 * Abort}
	 */
	...
	/**
	 * Détermination d'un éventuel gagnant ou d'un score atteint. Récuperation des
	 * données pour les statistiques et les classements. Snapshot éventuelle de la
	 * partie pour la sauvegarder lors de {@link Game#asyncManageGameData
	 * asyncManageGameData}. <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Stop Stop}
	 */
	protected abstract void collectGameData(Game<GD> game);

	/**
	 * Envoi <strong>asynchrone</strong> des statistiques vers les serveurs et mise
	 * à jour des classements. Cette phase peut aussi être utilisée pour sauvegarder
	 * de manière asynchrone la partie pour la relancer plus tard. <br>
	 * <br>
	 * <strong>Cette methode ne doit PAS utiliser l'API Bukkit.</strong> <br>
	 * <br>
	 * Cette methode est appelée par {@link Game#Stop Stop}
	 */
	...
	/**
	 * Methode appelée juste après le retrait d'un joueur. Cette methode est utile
	 * pour arrêter la partie ou équilibrer les équipes si la partie est en cours.
	 * 
	...
	/**
	 * Obtient la clef utilisée lors de l'enregistrement si le lobby a bien été
	 * enregistré.
	 * 
	 * @return
	 */
