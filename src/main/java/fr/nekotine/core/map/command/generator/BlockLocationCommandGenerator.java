...
/**
 * Générateur de commande pour un champ de type {@link org.bukkit.Location
 * Location} Il permet de définir les coordonées en arrondissant au block
 *
	...
	public MapCommandBranch[] generateFor(Function<CommandArguments, Object> pipeline, Class<?> elementType) {
		...
		MapCommandExecutor worldEditExecutor = (element, sender, args) -> {
			...
			try {
				...
				var e = (BlockVector) element;
				if (bb.getMinimumX() != bb.getMaximumX() || bb.getMinimumY() != bb.getMaximumY()
				...
				sender.sendMessage(
						Component.text("Vous devez sélectionner un block avec world edit.", Colors.Command.WARNING));
			}
			return element;
		};
