 ...
 *
 */
public class MapLocationElement implements ConfigurationSerializable{

	public MapLocationElement() {}
	
	public MapLocationElement(double x, double y, double z) {
		this.x = x;
		this.y = y;
		this.z = z;
	}
	
	public MapLocationElement(Location location) {
		x = location.getX();
		y = location.getY();
		z = location.getZ();
	}
	...
	public static MapLocationElement deserialize(Map<String,Object> map) {
		var x = (double)map.get("x");
		var y = (double)map.get("y");
		var z = (double)map.get("z");
		return new MapLocationElement(x, y, z);
	}
