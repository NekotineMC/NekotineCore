...
public enum DamageFunction {
	NEKOTINE(Constants.NEKOTINE_DAMAGE_EFFECT, Constants.NEKOTINE_DAMAGE_SOUND, Constants.NEKOTINE_DAMAGE_KNOCKBACK,
			Constants.NEKOTINE_DAMAGE_ENCHANTMENTS, Constants.NEKOTINE_DAMAGE_ARMOR,
			Constants.NEKOTINE_DAMAGE_VALIDATION, Constants.NEKOTINE_DAMAGE_CRITICAL), CUSTOM(Constants.NULL_CONSUMER,
					Constants.NULL_CONSUMER, Constants.NULL_CONSUMER, Constants.NULL_CONSUMER, Constants.NULL_CONSUMER,
					Constants.NULL_FUNCTION, Constants.NULL_TRICONSUMER);
	...
	DamageFunction(Consumer<LivingEntityDamageEvent> damageEffect, Consumer<LivingEntityDamageEvent> damageSound,
			Consumer<LivingEntityDamageEvent> damageKnockback, Consumer<LivingEntityDamageEvent> damageEnchantments,
			Consumer<LivingEntityDamageEvent> damageArmor, Function<LivingEntityDamageEvent, Boolean> damageValidation,
	...
	private static class Constants {
		...
		private static final Consumer<LivingEntityDamageEvent> NEKOTINE_DAMAGE_EFFECT = new Consumer<LivingEntityDamageEvent>() {
			...
			public void accept(LivingEntityDamageEvent e) {
				...
				e.GetDamaged().getWorld().spawnParticle(Particle.HEART, e.GetDamaged().getLocation().add(0, 1, 0), 5,
						0.5, 0, 0.5);
			}
		};
		private static final Consumer<LivingEntityDamageEvent> NEKOTINE_DAMAGE_SOUND = new Consumer<LivingEntityDamageEvent>() {
			...
			public void accept(LivingEntityDamageEvent e) {
				...
				e.GetDamaged().getWorld().playSound(e.GetDamaged().getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1f,
						1.5f);
			}
		};
		private static final Consumer<LivingEntityDamageEvent> NEKOTINE_DAMAGE_KNOCKBACK = new Consumer<LivingEntityDamageEvent>() {
			...
			public void accept(LivingEntityDamageEvent e) {
				if (e.IsKnockback() && (e.GetKnockbackOrigin() != null || e.GetDamager() != null)) {
					...
					EntityUtil.ApplyVelocity(e.GetDamaged(), trajectory, vel, false, 0, Math.abs(0.2 * knockback),
							0.4 + (0.04 * knockback), true);
				}
			}
		};
			...
			public void accept(LivingEntityDamageEvent e) {
				...
				for (ItemStack armor : e.GetDamaged().getEquipment().getArmorContents()) {
					if (armor != null) {
						...
						for (Enchantment enchant : enchants.keySet()) {
							...
							if (enchant.equals(Enchantment.PROTECTION))
							...
							else if (enchant.equals(Enchantment.FIRE_PROTECTION) && (e.GetCause() == DamageCause.FIRE
							...
							else if (enchant.equals(Enchantment.FEATHER_FALLING) && e.GetCause() == DamageCause.FALL)
								...
								if (e.GetDamager() != null)
									damageModule.Damage(e.GetDamager(), e.GetDamaged(), null, DamageCause.THORNS,
											THORNS_DAMAGE, false, true, null);
							}
						}
					}
				}
				...
				if (e.GetDamager() != null) {
					...
					if (e.GetDamager() instanceof Player)
						weapon = ((Player) e.GetDamager()).getInventory().getItemInMainHand();
					...
					if (weapon != null) {
						...
						for (Enchantment enchant : enchants.keySet()) {
							...
							if (enchant.equals(Enchantment.PUNCH) || enchant.equals(Enchantment.KNOCKBACK))
								...
								e.GetDamaged().setFireTicks(Math.max(e.GetDamaged().getFireTicks(),
										20 * ((enchants.get(enchant) * 4) - 1)));
							else if (enchant.equals(Enchantment.FLAME))
								e.GetDamaged().setFireTicks(
										Math.max(e.GetDamaged().getFireTicks(), 20 * ((enchants.get(enchant) * 5))));
		...
		private static final Consumer<LivingEntityDamageEvent> NEKOTINE_DAMAGE_ARMOR = new Consumer<LivingEntityDamageEvent>() {
			...
			public void accept(LivingEntityDamageEvent e) {
				...
				double withArmorAndToughness = e.GetDamage()
						* (1 - Math.min(20, Math.max(defense / 5, defense - e.GetDamage() / (2 + toughness / 4))) / 25);
		...
		private static final Function<LivingEntityDamageEvent, Boolean> NEKOTINE_DAMAGE_VALIDATION = new Function<LivingEntityDamageEvent, Boolean>() {
			...
			private static final DamageCause[] DAMAGE_CAUSE_WITH_INVINCIBILITY_FRAME = {DamageCause.CONTACT,
					DamageCause.FIRE, DamageCause.HOT_FLOOR, DamageCause.LAVA, DamageCause.SUFFOCATION};
			...
			public Boolean apply(LivingEntityDamageEvent e) {
				...
				if (Contains(DAMAGE_CAUSE_WITH_INVINCIBILITY_FRAME, e.GetCause())) {
					...
					if (e.GetDamaged().getNoDamageTicks() > 0)
						return false;
		...
		private static final TriConsumer<LivingEntityDamageEvent, Boolean, Boolean> NEKOTINE_DAMAGE_CRITICAL = new TriConsumer<LivingEntityDamageEvent, Boolean, Boolean>() {
			...
			public void accept(LivingEntityDamageEvent e, Boolean a, Boolean b) {
				if (a)
					e.SetDamage(e.GetDamage() / CRITICAL_BOOST);
				if (b)
					((AbstractArrow) e.GetProjectile()).setCritical(false);
			}
		};
	}
