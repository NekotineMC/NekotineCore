...
public class DamageModule implements IPluginModule, Listener {
	...
	public void OnDamage(EntityDamageEvent event) {
		...
		LivingEntityDamageEvent customDamageEvent = new LivingEntityDamageEvent(entity, damager, projectile,
				event.getCause(), damage, false, true, null);

		damageFunction.GetDamageCritical().accept(customDamageEvent, IsCritical(event),
				projectile instanceof AbstractArrow && ((AbstractArrow) projectile).isCritical());

		// If hit is valid (attack cooldown, ...)
		if (damageFunction.GetDamageValidation().apply(customDamageEvent))
	...
	public void EndEvent(LivingEntityDamageEvent event) {
		if (event.IsCancelled())
			return;
		if (!event.GetDamaged().isValid())
			return;
		if (event.GetDamaged().isInvulnerable())
			return;
	 ...
	 *
	 * @param damaged
	 *            L'entité qui prend les dégâts
	 * @param damager
	 *            L'entité qui fait les dégâts
	 * @param projectile
	 *            Le projectile qui fait lesdégâts
	 * @param cause
	 *            La cause des dégâts
	 * @param damage
	 *            La valeur des dégâts
	 * @param ignoreArmor
	 *            Si les dégâts doivent ignorer l'armure
	 * @param knockback
	 *            Si les dégâts doivent faire reculer
	 * @param knockbackOrigin
	 *            L'origine du recul
	 * @return L'event crée
	 */
	public LivingEntityDamageEvent Damage(LivingEntity damaged, LivingEntity damager, Projectile projectile,
			DamageCause cause, double damage, boolean ignoreArmor, boolean knockback, Location knockbackOrigin) {
		LivingEntityDamageEvent e = new LivingEntityDamageEvent(damaged, damager, projectile, cause, damage,
				ignoreArmor, knockback, knockbackOrigin);
		e.callEvent();
		return e;
	}
	 ...
	 *            L'origine du dégâts
	 * @param toIgnore
	 *            Liste des entités à ignorer
	 * @return Tous les events crées
	 */
	public ArrayList<LivingEntityDamageEvent> Explode(LivingEntity damager, double radius, DamageCause cause,
			double damage, boolean ignoreArmor, boolean knockback, Location origin, LivingEntity[] toIgnore) {
	...
	private void Damage(LivingEntityDamageEvent event) {
		...
		// event.GetDamaged().setLastDamageCause(new
		// EntityDamageEvent(event.GetDamaged(),
		// event.GetCause(), event.GetDamage()));
		if (event.GetDamager() instanceof Player && !event.GetDamaged().equals(event.GetDamager()))
			event.GetDamaged().setKiller((Player) event.GetDamager());
