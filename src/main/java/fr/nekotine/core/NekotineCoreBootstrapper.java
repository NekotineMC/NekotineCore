        ...
        context.getLifecycleManager().registerEventHandler(LifecycleEvents.DATAPACK_DISCOVERY.newHandler(
                evt -> {
                	try {
                		...
                		context.getLogger().error(Component.text("Impossible de charger le datapack avec l'URI suivant: "+datapackUri.toString(), NamedTextColor.RED), e);
                	}
                }
            ));
	}
        ...
        context.getLifecycleManager().registerEventHandler(LifecycleEvents.DATAPACK_DISCOVERY.newHandler(
                evt -> {
                	try {
                		...
                		context.getLogger().error(Component.text("Impossible de charger le datapack contenu dans le jar au chemin suivant: "+finalpath, NamedTextColor.RED), e);
                	}
                }
            ));
	}
