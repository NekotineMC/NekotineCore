...
public class PlayerInventorySnapshot implements Snapshot<PlayerInventory> {
	...
	private static EnumSet<EquipmentSlot> savedSlots = EnumSet.of(EquipmentSlot.HEAD, EquipmentSlot.CHEST,
	...
	public Snapshot<PlayerInventory> deepSnapshot(PlayerInventory inventory) {
		content = Arrays.asList(inventory.getContents()).stream().map(base -> base != null ? base.clone() : base)
