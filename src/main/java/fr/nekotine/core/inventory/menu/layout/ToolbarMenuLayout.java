...
public class ToolbarMenuLayout extends MenuLayout {
	...
	public void draw(Inventory inventory, int x, int y, int width, int height) {
		...
		for (var item : tools) {
			...
			inventory.setItem(InventoryUtil.chestCoordinateToInventoryIndex(curX, directionalY(curY, y, height)),
					item.draw());
			curX++;
		}
		...
		InventoryUtil.paintRectangle(inventory, brush, curX, directionalY(curY, y, height), curX + width,
				directionalY(curY, y, height));
		child.draw(inventory, curX, directionalY(++curY, y, height), width, height - (curY - y));
	}

	/**
	 * Retourne la valeur de Y selon le haut ou le bas du composant, selon la
	 * direction selectionnée
	 *
	 * @param y
	 * @param baseY
	 * @param height
