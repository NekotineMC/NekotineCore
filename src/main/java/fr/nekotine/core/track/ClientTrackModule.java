...
public class ClientTrackModule implements IPluginModule {
	...
	/*
	 * private static final byte ENTITY_ACTION_INDEX = 0; private static final byte
	 * SNEAKING_MASK = 0x02; private static final byte SPRINTING_MASK = 0x08;
	 */
	...
	private PacketListener receiverAdapter = new PacketAdapter(Ioc.resolve(JavaPlugin.class),
			PacketType.Play.Client.POSITION, PacketType.Play.Client.POSITION_LOOK, PacketType.Play.Client.ENTITY_ACTION,
			PacketType.Play.Client.LOOK, PacketType.Play.Client.GROUND, PacketType.Play.Client.HELD_ITEM_SLOT) {
	...
	/*
	 * private PacketListener sendingAdapter = new
	 * PacketAdapter(Ioc.resolve(JavaPlugin.class),
	 * PacketType.Play.Server.ENTITY_METADATA) { public void
	 * onPacketSending(PacketEvent event) { Player player = event.getPlayer(); if
	 * (map.containsKey(player)) {
	 *
	 * var newPacket = event.getPacket(); var metadatas =
	 * newPacket.getDataValueCollectionModifier().read(0); var filteredMetadatas =
	 * metadatas.stream().filter(v -> v.getIndex() == ENTITY_ACTION_INDEX).toList();
	 * if(filteredMetadatas.size() > 0) { var metadata = filteredMetadatas.get(0);
	 * var infos = map.get(player); //for(var metadata : filteredMetadatas) {
	 *
	 * var value = (byte)metadata.getRawValue(); value = (byte) (infos.isSneaking ?
	 * (value | SNEAKING_MASK) : (value & ~SNEAKING_MASK)); value = (byte)
	 * (infos.isSprinting ? (value | SPRINTING_MASK) : (value & ~SPRINTING_MASK));
	 ...
	 *
	 *
	 * //modifier.write(0,metadatas); event.setPacket(newPacket); } } } };
	 */
	...
	private void sendActionPacket(Player player, int actionId) {
		...
		Bukkit.getOnlinePlayers().stream().filter(p -> !p.equals(player))
				.forEach(p -> pmanager.sendServerPacket(p, metadataPacket));
	}
}
